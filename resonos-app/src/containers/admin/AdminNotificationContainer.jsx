    ...
    <div className="bg-dark text-light">
      <main className="py-5 bg-resonos-dark">
        <div className="container" style={{ maxWidth: "1100px" }}>
          ...
          {/* 알림 등록 폼 */}
          <section className={`${styles["resonos-card"]} p-4 mb-5`}>
            ...
            <form onSubmit={handleCreateNotification} className="row g-3">
              ...
              <div className="col-md-6">
                ...
                <input type="text" name="targetId" className="form-control"
                  value={formData.targetId} onChange={handleInputChange} />
              </div>
          ...
          {/* 검색 */}
          <SearchForm initialKeyword={keyword} placeholder="제목/내용 검색" onSearch={handleSearch} />

          {/* 리스트 */}
          <section className={`${styles["resonos-card"]} p-3`}>
            <TableColumnHeader columns={columns} />
            {notifications.length > 0 ? (
              notifications.map((noti) => (
                <div key={noti.id} className="list-group-item bg-dark text-light border-secondary mb-2 d-flex flex-nowrap align-items-center text-center">
                  <div style={columns[0].style}>{noti.id}</div>
                  <div style={columns[1].style}>{noti.userId}</div>
                  <div style={columns[2].style}>{noti.type}</div>
                  <div style={columns[3].style}>{noti.message}</div>
                  <div style={columns[4].style}>{noti.content}</div>
                  <div style={columns[5].style}>{noti.targetId || "-"}</div>
                  <div style={columns[6].style}>{noti.isRead ? "읽음" : "읽지 않음"}</div>
