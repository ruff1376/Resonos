          ...
          <div className="col-12 col-lg-6">
            <div className="resonos-card p-4 text-start">
              ...
              <form className="border rounded p-3 bg-dark mb-4" onSubmit={handleCreatePolicy}>
                <FormInput label="정책명" name="type" value={newPolicy.type}
                  onChange={(e) => setNewPolicy({ ...newPolicy, type: e.target.value })}
                  required containerClassName="mb-2" />
                <FormInput label="버전" name="version" value={newPolicy.version}
                  onChange={(e) => setNewPolicy({ ...newPolicy, version: e.target.value })}
                  containerClassName="mb-2" />
                <div className="mb-2">
                  <label className="form-label">내용</label>
                  <textarea className="form-control" rows="5"
                    value={newPolicy.content} onChange={(e) => setNewPolicy({ ...newPolicy, content: e.target.value })} />
                </div>
              ...
              {policies.map((p) => (
                <div key={p.id} className="policy-list mb-4">
                  ...
                  <div className="d-flex justify-content-between align-items-center">
                    ...
                    <button className="btn btn-outline-gold btn-sm" onClick={() => {
                  ...
                  {editPolicyId === p.id ? (
                    <div className="mt-2">
                      <FormInput label="정책명" value={editPolicy.type}
                        onChange={(e) => setEditPolicy({ ...editPolicy, type: e.target.value })}
                        required />
                      <FormInput label="버전" value={editPolicy.version}
                      ...
                      <button className="btn btn-gold btn-sm me-2" onClick={() => handleSavePolicy(p.id)}>저장</button>
                      <button className="btn btn-outline-secondary btn-sm" onClick={() => setEditPolicyId(null)}>취소</button>
                      <button className="btn btn-outline-danger btn-sm ms-2" onClick={() => handleDeletePolicy(p.id)}>삭제</button>
                    </div>
                  ) : (
            ...
            <div className="resonos-card p-4 text-start">
              ...
              <form className="mb-4" onSubmit={handleSaveSettings}>
                <div className="mb-2">
                  ...
                  <input type="checkbox"
                    ...
                    onChange={(e) => setSettingForm({ ...settingForm, allow_registration: e.target.checked })}/>
                </div>
                <div className="mb-2">
                  <label className="me-3">커뮤니티 게시판 활성화</label>
                  <input type="checkbox"
                    checked={settingForm.community_board_enabled}
                    onChange={(e) => setSettingForm({ ...settingForm, community_board_enabled: e.target.checked })}/>
                </div>
                <div className="mb-2">
                  <label className="me-3">음악 데이터 외부 연동</label>
                  <input type="checkbox"
                    checked={settingForm.external_music_data_sync}
                    onChange={(e) => setSettingForm({ ...settingForm, external_music_data_sync: e.target.checked })}/>
                </div>
                <div className="mb-2">
                  <label>기본 테마</label>
                  <select className="form-select w-auto ms-2"
                    value={settingForm.default_theme}
                    onChange={(e) => setSettingForm({ ...settingForm, default_theme: e.target.value })}>
              ...
              <form className="mb-4" onSubmit={handleCreateSetting}>
                <div className="row g-2 align-items-center mb-2">
                  <div className="col"><input type="text" className="form-control" name="name" placeholder="설정명" required /></div>
                  <div className="col"><input type="text" className="form-control" name="description" placeholder="설명" /></div>
                  <div className="col"><input type="text" className="form-control" name="value" placeholder="값" required /></div>
                  <div className="col-auto"><button className="btn btn-outline-gold">+ 추가</button></div>
                </div>
              </form>
              ...
              <table className="table table-dark table-bordered align-middle">
                <thead>
                  <tr>
                    <th>ID</th><th>설명</th><th>설정값</th><th>수정일시</th><th>생성일시</th><th>관리</th>
                  </tr>
                </thead>
                <tbody>
                    ...
                    <tr key={s.id}>
                      ...
                      <td>
                        <button className="btn btn-outline-danger btn-sm"
                          onClick={() => handleDeleteSetting(s.id)}>삭제</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
