// containers/admin/AdminMusicContainer.jsx
...
import SpotifySearchWithResults from '../../components/admin/second/SpotifySearchWithResults';
    ...
    track: [
      ...
      { label: '인기', style: { flexBasis: '8%', minWidth: '50px' }, render: row => row.popularity },
      {
        label: '관리', style: { flexBasis: '14%', minWidth: '80px' }, render: (row) => (
          <>
            <button className="btn btn-outline-gold btn-sm me-1" onClick={() => handleEditClick(row)}>수정</button>
            <button className="btn btn-outline-danger btn-sm" onClick={() => handleDelete(row.id)}>삭제</button>
          </>
        )
      }
    ],
