...
const LikedTrackSection = ({isOwner, likedTrackList, countTrack, setLikedTrackList, userId, onLikeTrack, onSearchLikedTrack}) => {
  ...
  useEffect(() => {
    ...
    onSearchLikedTrack(debouncedKeyword, offsetRef, limitRef, loadingRef, allLoadedRef, 'ar')
  }, [debouncedKeyword, setLikedTrackList])

  return (
    <section className={`info-wrapper ${!isOwner ? 'go-center' : ''}`}>
      <div className="info-section track">
        <div className="title">
          <div>
            <h2 className="text-start">좋아요 한 트랙</h2>
            <span className="count">{countTrack}</span>
          </div>
        ...
        <ul className="ul-list track">
          {!likedTrackList || likedTrackList.length === 0
          ? (
...
export default LikedTrackSection